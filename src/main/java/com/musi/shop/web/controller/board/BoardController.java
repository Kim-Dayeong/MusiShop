...
// Read
    ...
    public ResponseEntity<String> deleteBoardPost(@PathVariable Long id, @AuthenticationPrincipal PrincipalDetail principalDetail) {
        // 게시글 ID로 해당 게시글 가져오기
        Optional<Board> optionalBoard = boardReporitory.findById(id);

        // 게시글이 존재하지 않으면 에러 응답
        if (optionalBoard.isEmpty()) {
            return new ResponseEntity<>("{\"success\": false, \"message\": \"게시글이 존재하지 않습니다.\"}", HttpStatus.NOT_FOUND);
        }

        // 로그인 안된 상태일때
        if (principalDetail == null || !principalDetail.isAccountNonExpired()) {
            return new ResponseEntity<>("{\"success\": false, \"message\": \"인증되지 않은 사용자입니다.\"}", HttpStatus.UNAUTHORIZED);
        }
        ...
        if (boardMember == null || !Objects.equals(principalDetail.getUsername(), boardMember.getUsername())) {
            return new ResponseEntity<>("{\"success\": false, \"message\": \"해당 게시글에 대한 권한이 없습니다.\"}", HttpStatus.FORBIDDEN);
        }
